...
- `action_resolution/`：將 Interpretation 轉成 Standard Action，分析可行性並建立 Rule Request。
- `rule_engine/`：只執行確定性骰子、修正值、AC/DC、傷害與 HP 計算。
...
Demo 會額外顯示 Raw Player Input、Standard Action、Action Category、Feasibility、Routed Rule、Missing Fields、Rule Result 與 Resolution Status。
